...
- `tasks/check_apic_cluster.yml` now writes a JSON report for every APIC
  cluster check, not just console output. Report contains the APIC name,
  a UTC `checked_at` timestamp, the expected/actual fully-fit controller
  counts, and the full per-node detail (id, nodeName, addr, health, and
  every other field infraWiNode returns). Written to
  `{{ report_directory }}/{{ apic_name }}-cluster.json` -- same
  `report_directory` and `to_nice_json` pattern already used by the CIMC
  inventory reports, and the write location is announced before and after,
  matching the existing CIMC report tasks. Affects every playbook that
  calls this task: `03`, and all three call sites in `07`.
- Callers can set `apic_report_label` to write to a distinctly named file
  instead of the `cluster` default, if multiple checks in one run need to
  be kept separate rather than overwritten.
- No change to the fully-fit assertion logic or the APIC login/logout flow
  in this release.

## 1.3.10

- `tasks/check_apic_cluster.yml`'s per-node display now prints the **full**
  `infraWiNode` attribute set for every node (id, nodeName, addr, health,
  adminSt, operSt, apicMode, failoverStatus, podId, dn, and any other field
  the APIC returns), not just a curated id/name/address/health line. The
  1.3.9 curated line hid fields some environments care about; this shows
  everything infraWiNode reports, per node, alongside its health status.
- No change to the fully-fit assertion logic, `apic_cluster_output`, the
  APIC login/logout flow, or any CIMC/Redfish behavior in this release.

## 1.3.9

- `tasks/check_apic_cluster.yml` now reports **which** APIC node each health
  status belongs to, not just a flat list of health strings. The
  `infraWiNode` REST response includes `id`, `nodeName`, and `addr` per node
  (confirmed against a real APIC response shape), so the cluster check now
  keeps the full per-node attributes (`apic_cluster_nodes`) alongside the
  existing health-only list (`apic_cluster_output`, still used by the
  fully-fit assertion, unchanged).
- Added a one-line-per-node display task (`node 1 (apic1, 10.0.0.1):
  fully-fit`) plus a summary line (`apic01 cluster: 3 node(s) reported, 3
  fully-fit.`), replacing the old single debug line that only printed a bare
  list of health strings with no node identity.
- No change to the fully-fit assertion logic, APIC login/logout flow, or any
  CIMC/Redfish behavior in this release.

## 1.3.8

- **Reverted the 1.3.7 `ansible-core` re-pin.** 1.3.7 changed the pin from
  `2.13.13` to `2.21.3` based on a control node Python version (3.14.4) that
  turned out to belong to a test/dev machine, not the actual production
  control node. The production control node runs **Python 3.8**, which is
  the constraint this repo has to satisfy -- `ansible-core==2.13.13` (the
  1.3.3-1.3.6 pin) is correct for it and has been restored.
- Added a new README subsection, "Testing on a machine whose default Python
  isn't 3.8," covering the actual problem behind the 1.3.7 confusion: the
  `ast.Str`-removed-in-Python-3.14 crash only happens when a venv is built
  against a control node's non-3.8 default Python. It won't occur on the
  real Python 3.8 production node. Includes deadsnakes-PPA and pyenv
  instructions for installing a real Python 3.8 interpreter alongside a
  newer default Python, so test machines can accurately mirror production
  instead of hitting a false-negative crash from a version mismatch.
- No change to CIMC/APIC/Redfish/REST behavior, report contents, or
  credential prompting in this release.

## 1.3.7

- **Fixed a control-node crash**: `Verify required CIMC variables` (and any
  other task with a plain conditional such as `ansible_host is defined`)
  failed with `Invalid conditional detected: module 'ast' has no attribute
  'Str'` on a real-world control node. Root cause: `ansible-core==2.13.13`
  (pinned in 1.3.3 through 1.3.6) evaluates some conditionals using Python's
  `ast.Str`, which was removed in **Python 3.14**. The 2.13.13 pin was based
  on an incorrect assumption that the control node was on Python 3.8; the
  actual control node reported `Python 3.14.4`.
- Re-pinned `ansible-core` to **2.21.3**, the current actively maintained
  release supporting Python 3.12-3.14 on the control node (see the
  [ansible-core support matrix](https://docs.ansible.com/projects/ansible-core/devel/reference_appendices/release_and_maintenance.html)).
  This also moves the project off an EOL, unpatched `ansible-core` release.
- Re-verified every playbook's `--syntax-check` against the newest
  `ansible-core` installable in this project's own Python 3.10 verification
  environment (2.17.14) with zero errors or deprecation warnings; confirmed
  via the official support matrix that 2.21.x covers the same Python
  3.12-3.14 range the real control node reported.
- Rewrote the README's "Python / ansible-core compatibility note" to state
  the actual control node requirement (Python 3.12+) instead of the
  incorrect Python 3.8 constraint, and to tell readers to confirm their
  Python version with `python3 --version` before installing rather than
  assume it.
- No change to CIMC/APIC/Redfish/REST behavior, report contents, or
  credential prompting in this release -- this is purely an
  ansible-core/Python compatibility fix.
