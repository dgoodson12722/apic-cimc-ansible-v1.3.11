...
- Python 3.8 on the control node (this is the actual production control
  node's Python version -- see the compatibility note below if you're
  developing or testing on a machine with a different Python version)
...
**The production control node for this project runs Python 3.8.** As of this
...
support the actual production control node's Python version. Every playbook
in this repo has been syntax-checked and re-verified to run correctly
against 2.13.13 specifically (not just a newer version). If/when the
production control node's Python is upgraded past 3.8, move to a currently
supported `ansible-core` release (see the
...
Install Ansible in a virtual environment, matching production's Python:
...
**WSL/Ubuntu note:** if `python3.8 -m venv .venv` fails with a message
telling you to install a `python3.X-venv` package, the `venv` module isn't
bundled with your distro's Python by default. Install it, then retry:

```bash
sudo apt update
sudo apt install python3.8-venv   # match the version number in the error
```

#### Testing on a machine whose default Python isn't 3.8

If you're developing or testing on a machine whose only Python is newer than
3.8 (for example, a workstation on Python 3.12+), **do not** create the venv
with that default `python3` -- `ansible-core==2.13.13` evaluates some plain
conditionals (including simple ones like `ansible_host is defined`) using
Python's `ast.Str`, which was removed in Python 3.14. On a control node
newer than what 2.13.13 supports, tasks can fail immediately with
`AttributeError: module 'ast' has no attribute 'Str'` (or similar) before any
CIMC or APIC is contacted -- this is a test-environment mismatch, not a bug
in this repo, and it will not happen on the actual Python 3.8 production
control node.

To test accurately, install a real Python 3.8 interpreter alongside your
system Python and point the venv at it explicitly. On Ubuntu/WSL, the
[deadsnakes PPA](https://launchpad.net/~deadsnakes/+archive/ubuntu/ppa)
provides EOL Python versions that Ubuntu no longer ships:
...
If `add-apt-repository` isn't available or your distro isn't Ubuntu-based,
[pyenv](https://github.com/pyenv/pyenv) works cross-distro:
`pyenv install 3.8.18 && pyenv local 3.8.18`, then create the venv as above.

Confirm you're actually on 3.8 before installing anything:
...
Every APIC cluster health check (playbook `03`, and each of the three cluster
checks inside `07`) also writes a report to
`reports/{{ apic_name }}-cluster.json`, containing the expected/actual
fully-fit counts and the full per-node `infraWiNode` detail (id, nodeName,
address, health, and every other field the APIC returns).
...
This repository uses semantic versioning. The current release is `1.3.11`.
